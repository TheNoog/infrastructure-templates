...
# Run as part of deployment to build pickle files.

def load_manifest():
    """Load manifest.json """
    #local_filepath = f"{DBT_DIR}/target/manifest.json"
    local_filepath = f"./target/manifest.json"
...
def load_model_selectors():
    ...
    #with open(f"{DBT_DIR}/selectors.yml-1.10.12") as f:
    with open(f"./selectors.yml") as f:
...
def parse_model_selector(selector_def):
    ...
    models = os.popen(f"cd ./ && dbt ls --models {selector_def}").read()
